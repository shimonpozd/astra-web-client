...
export function buildTimelineBlocks({ people, periods, activePeriodId }: BuildParams): PeriodBlock[] {
  ...
  const buildBlock = (period: Period, x: number, y: number, widthOverride?: number): PeriodBlock => {
    ...
    if (period.id === 'malakhim_divided') {
      ...
      if (timelinePeople.length) {
        // Все с датами в одном пространстве, цвет различается через timelineColors по subPeriod/region
